...
Normal windows share an application WebContext, so their browser session
survives destroy/recreate within the process. Routes and native capability
policies remain template-specific. Incognito WebViews are an exception: Wry
ignores the shared context for them, so cross-window incognito session sharing
is platform-dependent and not guaranteed.
