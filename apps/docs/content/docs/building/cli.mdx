...
- **`--sign`** — Developer ID-sign the `.app` on macOS or Authenticode-sign
  ...
  signing must run on Windows with SignTool. See [Distribution](/docs/building/distribution#signing--notarization).
...
- **`--sign`** — macOS Developer ID or Windows Authenticode signing. On
  Windows it signs and verifies the app executable, NSIS setup, and MSI.
...
- **`--manifest`** — scans `dist/` for this version's payloads (the macOS
  ...
  `-setup.exe` name is still recognized for win32-x64), hashes whichever
  exist, and writes `dist/latest.json` with a `generatedAt` timestamp.
