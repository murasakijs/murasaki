...
`sign.appSandbox` (default `false`) controls how `murasaki bundle --sign`'s
generated default entitlements plist reacts to declared `systemPermissions`:
with it off (Murasaki's default hardened-runtime-only posture), only
`appleEvents` adds anything (`com.apple.security.automation.apple-events`) —
every other kind works from its Info.plist usage string alone. With it on,
`com.apple.security.app-sandbox` plus each declared kind's documented App
Sandbox entitlement are added too (`photos` has none and is never added). This
derivation only applies to the generated default plist — a custom
`sign.entitlements` file is always used verbatim. See
...
| `entitlements` | `string?` | Path to a custom entitlements `.plist`. Defaults to a Node-friendly hardened-runtime set (JIT + unsigned executable memory + disabled library validation) plus a derivation from declared `systemPermissions` — see [`systemPermissions` → Entitlements and the App Sandbox](#systempermissions). |
| `appSandbox` | `boolean?` | Opt into the macOS App Sandbox in the generated default entitlements plist. Default `false`. |
