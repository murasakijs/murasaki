...
Consumed by `useUpdate()` (from `murasaki`) and `<UpdateButton />` (from
`@murasakijs/ui`) — see the full [Auto-update](/docs/guides/auto-update)
guide for the manifest format, signing, and publishing releases.
