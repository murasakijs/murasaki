...
<Callout type="warn">
  Linux では `Copy`/`Cut`/`Paste`/`SelectAll` は X11 キーイベントの合成によって実現され
  ます(Wayland では XWayland が必要)。一方 `Undo`/`Redo` はメニューから静かに省かれま
  す — muda/GTK 側の既知の制約です。Linux でこれらが必要な場合は、独自の `action` 関数
  を指定してください。
</Callout>
