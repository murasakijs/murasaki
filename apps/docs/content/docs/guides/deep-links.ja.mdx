...
Murasaki は packaged macOS / Windows / Linux app の custom URL scheme と
document extension を登録できます。cold launch と起動済み app への request は、
どちらも同じ Node Main の `openRequested()` hook に届きます。
...
| Linux AppDir / `.AppImage` / `.deb` | `murasaki bundle` が `x-scheme-handler/<scheme>` と `application/x-<extension>` の `MimeType=` エントリを `.desktop` ファイルに書き込みます。`.deb` は `usr/share/applications/` にインストールし、インストール/削除時に desktop database を更新します(`update-desktop-database`)。手動展開した AppDir/`.AppImage` には同等の OS レベル登録手順はありませんが、直接起動した場合でも cold-start 時の argv(`.desktop` の `Exec=` 行が展開する `%U`/`%F`)と 2 回目起動時の activation は動作します。 |
