...
2. アップデーターを有効化します:
   ...
   フィールドから、公開鍵は `.murasaki/update-key.pub` から推論されます。
...
`<UpdateButton />`(`@murasakijs/ui`)は、それをラップしたすぐに使える表示レイヤーで
す:
...
`checkOnStart` と `checkInterval` も設定から解決されますが、現時点ではそれらから実際
にチェックをスケジュールする仕組みはありません — 今日 `check()` を呼び出しているの
は、`<UpdateButton />` 自身のマウント時の効果だけです。それ以上のチェックが必要な場
合は、`useUpdate().check()` を(たとえば自分のタイマーから)自分で呼び出してくださ
い。
