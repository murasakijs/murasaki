...
test('navigation and tab primitives preserve baseline ARIA semantics during SSR', () => {
  const html = renderToStaticMarkup(
    h('div', null,
      h(UI.Breadcrumb, { 'aria-label': 'Breadcrumb' },
        h(UI.BreadcrumbList, null,
          h(UI.BreadcrumbItem, null, h(UI.BreadcrumbLink, { href: '/' }, 'Home')),
          h(UI.BreadcrumbSeparator, null),
          h(UI.BreadcrumbItem, null, h(UI.BreadcrumbPage, null, 'Docs')),
        ),
      ),
      h(UI.Tabs, { defaultValue: 'preview' },
        h(UI.TabsList, null,
