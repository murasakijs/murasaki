...
impl Drop for X11Data {
  fn drop(&mut self) {
    // Only tear down the X window wry created itself (the is_child container).
    // In the non-child embedding path x11_window/gtk_window alias the PARENT
    // tao window, which tao owns and destroys — destroying it here raced tao's
    // own teardown into an X11 BadWindow that aborted the process. Every other
    // method in this file guards on is_child; the original Drop did not.
    // Upstreamed to tauri-apps/wry (see crates/native/vendor/wry/PATCH.md).
    if self.is_child {
