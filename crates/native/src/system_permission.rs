...
#[cfg(target_os = "macos")]
use objc2_core_foundation::{CFBoolean, CFDictionary, CFType, CFRetained};
#[cfg(target_os = "macos")]
...
fn capture_usage_description_present(name: &str) -> bool {
  ...
  NSBundle::mainBundle().objectForInfoDictionaryKey(&key).is_some()
}
...
pub(crate) fn status(name: &str) -> Result<&'static str, String> {
  ...
  #[cfg(target_os = "macos")]
  {
    if matches!(name, "camera" | "microphone") {
      let media_type = capture_media_type(name)
        .ok_or_else(|| format!("macOS did not expose the {name} media permission constant"))?;
  ...
  #[cfg(target_os = "macos")]
  {
    ...
    return match name {
      ...
      "accessibility" => {
        ...
        let options: CFRetained<CFDictionary> = unsafe {
          CFRetained::cast_unchecked(typed)
        };
